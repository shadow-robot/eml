...
#include "eml/ethercat_log.h"
#include "eml/ethercat_AL.h"
#include "eml/ethercat_slave_conf.h"
#include "eml/ethercat_slave_memory.h"
#include "eml/ethercat_slave_handler.h"
#include "eml/ethercat_dll.h"
#include "eml/ethercat_telegram.h"
...
#include "eml/ethercat_process_data.h"
#include <time.h>
...
EtherCAT_AL::EtherCAT_AL(EtherCAT_DataLinkLayer *_m_dll_instance,
  ...
  m_ready(false)
{
  assert(m_dll_instance);
  assert(m_logic_instance);
  assert(pd_buffer_);
  if (!init())
  {
...
bool EtherCAT_AL::scan_slaves(void)
{
  ...
  ec_log(EC_LOG_INFO, "EtherCAT AL: Ring contains %u slaves\n",m_num_slaves);
  ...
  for (unsigned int i = 0; i < m_num_slaves; i++)
  {
    ...
    if (!succeed)
    {
      ec_log(EC_LOG_FATAL,"EC_AL::scan_slaves() Error reading Product code of slave %u\n",i);
    ...
    if (!succeed)
    {
      ec_log(EC_LOG_FATAL,"EC_AL::scan_slaves() Error reading Revision of slave %u\n",i);
      revision = 0xbaddbadd;
    }
    else
    {
    ...
    if (!succeed)
    {
      ec_log(EC_LOG_FATAL,"EC_AL::scan_slaves() Error reading Serial of slave %u\n",i);
      serial = 0xbaddbadd;
    }
    else
    {
